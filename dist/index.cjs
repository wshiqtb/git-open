...
const isWsl = () => {
	...
	try {
		return node_fs.default.readFileSync("/proc/version", "utf8").toLowerCase().includes("microsoft") ? !isInsideContainer() : false;
	} catch {
		return false;
	}
};
...
const execFile = node_util.default.promisify(node_child_process.default.execFile);
...
const getWslDrivesMountPoint = (() => {
	...
	return async function() {
		...
		try {
			await node_fs_promises.default.access(configFilePath$1, node_fs_promises.constants.F_OK);
			isConfigFileExists = true;
		} catch {}
		if (!isConfigFileExists) return defaultMountPoint;
		const configContent = await node_fs_promises.default.readFile(configFilePath$1, { encoding: "utf8" });
...
const baseOpen = async (options) => {
	...
	if (platform === "darwin") {
		...
		command = is_wsl_default ? await getPowershellPathFromWsl() : `${node_process.default.env.SYSTEMROOT || node_process.default.env.windir || "C:\\Windows"}\\System32\\WindowsPowerShell\\v1.0\\powershell`;
		...
		if (app) command = app;
			...
			const useSystemXdgOpen = node_process.default.versions.electron ?? (platform === "android" || isBundled || !exeLocalXdgOpen);
...
async function main() {
	...
	program.parse(node_process.default.argv);
